...
const Card = ({ image, name, hp, str, weak, happy, btn }) => {
	...
	const happyList = useMemo(() => {
		...
		for (let i = 1; i <= happy; i++) {
			list.push(
				<img key={i} className={css(style.cuteImage)} src={cute} alt="cute" />,
			);
		}
		return list;
	}, [happy]);
		...
		<div
			...
			<div className={css(style.desc)}>
				...
				<div className={css(style.descContainer)}>
					...
					<div className={css(style.descBar)}>
				...
				<div className={css(style.descContainer)}>
					...
					<div className={css(style.descBar)}>
						<ProgressBar value={str} />
					</div>
				</div>
				<div className={css(style.descContainer)}>
					<div className={css(style.descText)}>WEAK</div>
					<div className={css(style.descBar)}>
						<ProgressBar value={weak} />
					</div>
				</div>
				<div className={css(style.cuteContainer)}>{happyList}</div>
			</div>
		</div>
	);
};
